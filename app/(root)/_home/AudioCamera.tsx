...
function AudioCamera() {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 rounded border p-2">
      {audiocamera.map((acproduct, index) => (
        <div key={index} className="pt-2">
          <div className="bg-white border border-gray-300 hover:shadow-lg transition-shadow duration-200 rounded-md p-3">
            {/* Header */}
            <div className="flex justify-between items-center mb-4">
              <h1 className="text-lg font-semibold">{acproduct.title}</h1>
            ...
            <div className="grid grid-cols-2 gap-4">
              {acproduct.idata.map((item, idx) => (
                <div key={idx} className="flex flex-col items-center text-center">
                  ...
                  />
                  <h2 className="mt-2 text-sm font-medium">{item}</h2>
